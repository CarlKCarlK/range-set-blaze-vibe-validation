...
This module implements a deliberately simple reference overwrite.  It first
removes the overwritten part of every old run, retaining a left and/or right
residual where appropriate.  It then inserts the new run between those two
ordered lists and coalesces touching neighbors that carry equal values.
-/

/-- The weaker ordering needed before equal-valued touching runs are
coalesced: stored ranges are ordered and do not overlap. -/
private def Run.disjointBefore {Value : Type*} (a b : Run Value) : Prop :=
...
run list. -/
private def coalesce {Value : Type*} [DecidableEq Value] : List (Run Value) → List (Run Value)
  | [] => []
  | run :: rest =>
      match coalesce rest with
      | [] => [run]
      | next :: tail =>
...
private def overwriteRunsRaw {Value : Type*}
...
private lemma runsToFunction_leftResiduals {Value : Type*}
...
private lemma runsToFunction_rightResiduals {Value : Type*}
...
private lemma runsToFunction_append {Value : Type*}
...
private lemma overwriteRunsRaw_toFunction {Value : Type*}
    (runs : List (Run Value)) (input : IntRange) (value : Value)
    (h : input.lo ≤ input.hi) :
    runsToFunction (overwriteRunsRaw runs input value h) =
      overwrite (runsToFunction runs) input value := by
  funext key
  simp only [overwriteRunsRaw, runsToFunction_append,
    runsToFunction_leftResiduals, runsToFunction_rightResiduals,
    runsToFunction_cons]
...
private lemma coalesce_toFunction {Value : Type*} [DecidableEq Value]
    (runs : List (Run Value)) :
    runsToFunction (coalesce runs) = runsToFunction runs := by
  induction runs with
  | nil => simp [coalesce]
  | cons run rest ih =>
      cases hrest : coalesce rest with
      | nil =>
          funext key
          have hnone : runsToFunction rest key = none := by
            rw [← ih, hrest]
            rfl
          simp [coalesce, hrest, hnone]
      | cons next tail =>
          by_cases htouch : run.value = next.value ∧
              run.range.val.hi + 1 = next.range.val.lo
          · rw [show coalesce (run :: rest) =
                mergeTouchingRuns run next htouch.2 :: tail by
                  simp [coalesce, hrest, htouch]]
            rw [mergeTouchingRuns_toFunction run next tail htouch.2 htouch.1]
            funext key
            have htailFunction : runsToFunction (next :: tail) key =
                runsToFunction rest key := by
              rw [← hrest]
              exact congrFun ih key
            simp only [runsToFunction_cons]
            split <;> simp_all
          · rw [show coalesce (run :: rest) = run :: next :: tail by
                  simp [coalesce, hrest, htouch]]
            funext key
            have htailFunction : runsToFunction (next :: tail) key =
                runsToFunction rest key := by
            ...
            split
            · rfl
            · simpa only [runsToFunction_cons] using htailFunction

private lemma coalesce_head_lo {Value : Type*} [DecidableEq Value]
    (first : Run Value) (rest : List (Run Value))
    {head : Run Value} {tail : List (Run Value)}
    (hresult : coalesce (first :: rest) = head :: tail) :
    head.range.val.lo = first.range.val.lo := by
  unfold coalesce at hresult
  cases hrest : coalesce rest with
  | nil =>
      simpa [hrest] using
...
private lemma mergeTouchingRuns_before {Value : Type*}
  ...
  constructor
  · simpa [mergeTouchingRuns] using hbefore.1
  · intro heq
    apply hbefore.2
    simpa [mergeTouchingRuns, hvalue] using heq

private lemma coalesce_canonical {Value : Type*} [DecidableEq Value]
    (runs : List (Run Value))
    (hordered : List.Pairwise Run.disjointBefore runs) :
    Canonical (coalesce runs) := by
  induction runs with
  | nil => simp [coalesce, Canonical]
  | cons run rest ih =>
      have hcanonicalRest := ih hordered.tail
      cases hcoal : coalesce rest with
      | nil => simp [coalesce, hcoal, Canonical]
      | cons next tail =>
          have hnextTail : List.Pairwise Run.before (next :: tail) := by
            simpa [Canonical, hcoal] using hcanonicalRest
          cases rest with
          | nil => simp [coalesce] at hcoal
          | cons original remainder =>
              have hlo : next.range.val.lo = original.range.val.lo :=
                coalesce_head_lo original remainder hcoal
              ...
              by_cases htouch : run.value = next.value ∧
                  run.range.val.hi + 1 = next.range.val.lo
              · rw [show coalesce (run :: original :: remainder) =
                    mergeTouchingRuns run next htouch.2 :: tail by
                      simp [coalesce, hcoal, htouch]]
...
private lemma leftResiduals_pairwise {Value : Type*}
  ...
  apply hcanonical.filterMap (leftResidual start)
  ...
  exact lt_of_le_of_lt pleft.2.1 (hbefore.1.trans_eq pright.1.symm)
...
private lemma rightResiduals_pairwise {Value : Type*}
  ...
  apply hcanonical.filterMap (rightResidual stop)
  intro left right hbefore left' hleft right' hright
  have pleft := rightResidual_properties hleft
  have pright := rightResidual_properties hright
  unfold Run.disjointBefore
  exact lt_of_eq_of_lt pleft.2.2.1 (hbefore.1.trans_le pright.1)
...
private lemma overwriteRunsRaw_pairwise {Value : Type*}
    (runs : List (Run Value)) (input : IntRange) (value : Value)
    (hnonempty : input.lo ≤ input.hi) (hcanonical : Canonical runs) :
    List.Pairwise Run.disjointBefore
      (overwriteRunsRaw runs input value hnonempty) := by
  ...
  have hleft : List.Pairwise Run.disjointBefore left :=
    leftResiduals_pairwise runs input.lo hcanonical
  have hright : List.Pairwise Run.disjointBefore right :=
    rightResiduals_pairwise runs input.hi hcanonical
...
def algoAMap {Value : Type*} [DecidableEq Value]
  ...
  if h : input.hi < input.lo then
    ...
    let runs := coalesce (overwriteRunsRaw map.runs input value hnonempty)
    refine ⟨runs, ?_⟩
    exact coalesce_canonical _
      (overwriteRunsRaw_pairwise map.runs input value hnonempty map.canonical)

/-- Algo A always returns a canonical run representation. -/
theorem algoAMap_canonical {Value : Type*} [DecidableEq Value]
    (map : RangeMapBlaze Value) (input : IntRange) (value : Value) :
    Canonical (algoAMap map input value).runs :=
  (algoAMap map input value).canonical

/-- A reversed overwrite is an exact representation-level no-op. -/
theorem algoAMap_eq_of_hi_lt_lo {Value : Type*} [DecidableEq Value]
    (map : RangeMapBlaze Value) (input : IntRange) (value : Value)
    (h : input.hi < input.lo) :
    algoAMap map input value = map := by
  simp [algoAMap, h]

/-- Algo A has exactly the pointwise semantics of overwriting the input
interval with the supplied value. -/
theorem algoAMap_toFunction {Value : Type*} [DecidableEq Value]
    (map : RangeMapBlaze Value) (input : IntRange) (value : Value) :
    (algoAMap map input value).toFunction = overwrite map.toFunction input value := by
  unfold algoAMap
  split <;> rename_i h
  · exact (overwrite_eq_of_hi_lt_lo map.toFunction input value h).symm
  · dsimp only [toFunction]
    rw [coalesce_toFunction]
    apply overwriteRunsRaw_toFunction
